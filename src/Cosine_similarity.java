...
public class Cosine_similarity {
	...
	static Double Sen_weight[]; // weight of each sentence to be used in
								// denominator
	...
	Cosine_similarity(int matrix_size, String tempSen[][]) // method for
															// initializing
															// similarity matrix
	{
	...
	void setSenWeight(HashMap<String, Entry> map) // method for calculating
													// weight of each sentence
	{
		...
		for (int i = 0; i < temp.length; i++) {
			...
			for (int j = 0; j < temp[i].length; j++) {
				...
				Sen_weight[i] += Math.pow(
						getFqInSen(temp[i][j], i) * e1.get_idf(), 2);
	...
	int getFqInSen(String word, int Sno) // method to get the frequency of a
											// word in a sentence.
	{
		int count = 0;
	...
	void setSimMatrix(HashMap<String, Entry> map) {
		...
		for (int p = 0; p < sim_matrix.length; p++) {
			for (int q = p; q < sim_matrix.length; q++) {
				HashMap<String, Entry> commonWords = new HashMap<String, Entry>(); // list
																					// of
																					// common
																					// words
																					// in
				...
				if (p == q) {
					...
					for (int r = 0; r < temp[p].length; r++) {
						if (temp[p][r].equals(null) || temp[p][r].equals("")) // check
																				// null
																				// words
							continue;
						if (inSen2(temp[p][r], q)) // check if a word is present
													// in both sentences p and q
						{
					...
					sim_matrix[p][q] = sim_matrix[q][p] = sumN
							/ (Math.sqrt(Sen_weight[p]) * Math
									.sqrt(Sen_weight[q]));
